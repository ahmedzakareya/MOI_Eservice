...
namespace MOINFO_API.Controllers
{
    [Route("api/Mosanafat")]
    [ApiController]
    public class APIMosanafatController : ControllerBase
    {
        ...
        public APIMosanafatController(IUnitOfwork unitOfwork, IUpdateDataService updateDataService, IMapper mapper, IDataFetchService dataFetchService)
        {
    ...
    .ToListAsync();
        ...
        {
            var entities = await _unitOfwork.genericRepository<AttachRule>().GetByCondition(c => c.ViewType == ViewType && c.FlagView == "user" && c.ServiceId == 5 )
    .ToListAsync();
        ...
        public async Task<MoiEserviceLicensesRequestVM> AddNewCompanyRequest([FromBody] MoiEserviceLicensesRequestVM model)
        {
            try
            {
                ...
                var licenseInfo = await _unitOfwork.genericRepository<MoiEserviceLicenseInfo>()
                    .GetByCondition(c => c.ActvityTypeId == model.ActivityTypeId && c.ServiceId == 5 && c.ReqTypeId == 1)
                    .FirstOrDefaultAsync();
                ...
                // الطلب
                long? Sequence = await GetNextRequestIdForService(5);
        ...
        public async Task<WorkFlowVM> GetFlowVMAsync(int requestId)
        {
            try
            {
                const int serviceId = 5; // Fixed service id
    ...
    .GetAllAsync();
                ...
                var ChangeAddessFees  = await _unitOfwork.genericRepository<MoiEserviceLicenseInfo>()
                                .GetByCondition(c => c.ActvityTypeId == vm.ActiivityTypeId && c.ServiceId == 5  && c.TransTypeId == 4)
                                .AsNoTracking()
                                .Select(c => c.FixedFees)
                                .FirstOrDefaultAsync(ct);
                var RenewFees = await _unitOfwork.genericRepository<MoiEserviceLicenseInfo>()
                                .GetByCondition(c => c.ActvityTypeId == vm.ActiivityTypeId && c.ServiceId == 5 && c.TransTypeId == 17)
                                .AsNoTracking()
                                .Select(c => c.FixedFees)
                                .FirstOrDefaultAsync(ct);
                var ReplaceOfLostFees = await _unitOfwork.genericRepository<MoiEserviceLicenseInfo>()
                                .GetByCondition(c => c.ActvityTypeId == vm.ActiivityTypeId && c.ServiceId == 5 && c.TransTypeId == 12)
                                .AsNoTracking()
                                .Select(c => c.FixedFees)
                                .FirstOrDefaultAsync(ct);

                var ChangePartnerFees = await _unitOfwork.genericRepository<MoiEserviceLicenseInfo>()
                                .GetByCondition(c => c.ActvityTypeId == vm.ActiivityTypeId && c.ServiceId == 5 && c.TransTypeId == 3)
                                .AsNoTracking()
                                .Select(c => c.FixedFees)
                                .FirstOrDefaultAsync(ct);

                var changeCompanyNameFess = await _unitOfwork.genericRepository<MoiEserviceLicenseInfo>()
                                .GetByCondition(c => c.ActvityTypeId == vm.ActiivityTypeId && c.ServiceId == 5 && c.TransTypeId == 1)
                                .AsNoTracking()
                                .Select(c => c.FixedFees)
                                .FirstOrDefaultAsync(ct);

                var ChangeCommercialNameFees = await _unitOfwork.genericRepository<MoiEserviceLicenseInfo>()
                                .GetByCondition(c => c.ActvityTypeId == vm.ActiivityTypeId && c.ServiceId == 5 && c.TransTypeId == 2)
                                .AsNoTracking()
                                .Select(c => c.FixedFees)
                                .FirstOrDefaultAsync(ct);
                ...
                var ChangeManagerFees = await _unitOfwork.genericRepository<MoiEserviceLicenseInfo>()
                                .GetByCondition(c => c.ActvityTypeId == vm.ActiivityTypeId && c.ServiceId == 5 && c.TransTypeId == 9)
                                .AsNoTracking()
                                .Select(c => c.FixedFees)
                                .FirstOrDefaultAsync(ct);
                var RenouncementFees = await _unitOfwork.genericRepository<MoiEserviceLicenseInfo>()
                               .GetByCondition(c => c.ActvityTypeId == vm.ActiivityTypeId && c.ServiceId == 5 && c.TransTypeId == 18)
                               .AsNoTracking()
                               .Select(c => c.FixedFees)
                               .FirstOrDefaultAsync(ct);
        ...
        public async Task<IEnumerable<object>> GetRequestsWithLicNoAndStatus(int serviceId)
        {
            try
            {
                ...
                var requestsWithDetails = requests
                    .Where(r => r.ServiceId == 5)
                    .Select(r => new
                    {
                        r.RequestId,
                        r.Reqno,
                        r.ServiceId,
                        r.LicenseId,r.ReqtypeId ,
        ...
        public async Task<ActionResult<WorkFlowVM>> FetchWorkFlowVM(int? requestID)
        {
            ...
            // 2) Short-circuit statuses (no workflow query)
            if (request.RequestStatusId is
                (int)RequestStatusEnum.FinalLicenseIssued or
                (int)RequestStatusEnum.RequestDeclined or (int)RequestStatusEnum.RequestRevokedToCompleteData)  
            {
                return Ok(new WorkFlowVM
                {
                    CurrentStatusId = request.RequestStatusId
                });
            }

            // 3) Query workflow normally for other statuses
            ...
            // Keep current aligned with request
            model.CurrentStatusId = request.RequestStatusId;

            // 5) Extra logic for MOIT status
            if (request.RequestStatusId == (int)RequestStatusEnum.WaitingForMOIT)
            {
                ...
                model.IsMOCIData = company != null &&
                    (!string.IsNullOrWhiteSpace(company.Name) ||
                     !string.IsNullOrWhiteSpace(company.RecordNo));
            }
