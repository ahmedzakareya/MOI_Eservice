...
{
    public class MosanafatController : Controller
    {
        private readonly ILogger<MosanafatController> _logger;
        ...
        public MosanafatController(IConfiguration configuration, ILogger<MosanafatController> logger, HelperUrlApi helperUrlApi, HttpClient httpClient)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> PressNewRequestCompany(MoiEserviceLicensesRequestVM model)
        {
            ...
            if (result != null)
            {
                ...
                var baseFolder = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Files", "Mosanafat");
        ...
        private async Task<MoiEserviceLicensesRequestVM?> SendCompanyRequestAsync(MoiEserviceLicensesRequestVM model)
        {
            try
            {
                var apiUrl = $"{_baseUrl}api/Mosanafat/AddNewCompanyRequest";
        ...
        public async Task<IActionResult> PressNewRequestPersonal()
        {
            MoiEserviceLicensesRequestVM model = new MoiEserviceLicensesRequestVM()
            {
                ...
                LicenTypeName = "مؤسسة",
                countriesLookups = await FetchCountriesAsync(),
                attachRules = await FetchAttachmentsAsync("NewPersonalRequestMosanafat"),
                ActivityTypes = await FetchActivitiesAsync(5),
                pesronTypeLookUp = await FetchPesronTypesAsync(),
                qualificationsLookups = await FetchQualificationsAsync(),
        ...
        private async Task<List<ActivityTypeVM>> FetchActivitiesAsync(int id)
        {
            try
            {
                var requestUrl = $"api/Mosanafat/GetActivity?ID={id}";
        ...
        private async Task<List<TestablishContractVM>> FetchTestablishContractAsync()
        {
            try
            {
                var requestUrl = $"api/Mosanafat/GetTestablishContract";
        ...
        private async Task<List<PesronTypeLookUpVM>> FetchPesronTypesAsync()
        {
            try
            {
                var requestUrl = $"api/Mosanafat/GetPesronTypes";
        ...
        private async Task<List<QualificationsLookupVM>> FetchQualificationsAsync()
        {
            try
            {
                var requestUrl = $"api/Mosanafat/GetQualificationsLookup";
        ...
        private async Task<List<Business.ViewModel.AttachRuleVM>> FetchAttachmentsAsync(string viewType)
        {
            try
            {
                var requestUrl = $"api/Mosanafat/GetAttachmentForRequest?viewType={viewType}";
        ...
        public async Task<IActionResult> RequestDetails(string id)
        {
            try
            {
                ...
                if (int.TryParse(MyCrypto.Decode(id), out ReqID))
                {
                    ...
                    var apiSetting = _baseUrl + $"api/Mosanafat/GetRequestDetails/{ReqID}";
        ...
        private async Task<LicenseModifyModel> FetchLicenseModifyModelAsync(int licenseID)
        {
            try
            {
                var requestUrl = $"api/Mosanafat/LicenseModifyModel?licenseID={licenseID}";
        ...
        //public ActionResult LicenseModifyRequest(LicenseModifyModel model)
        //{
        //    if (model == null)
        //    {
        ...
        public async Task<IActionResult> LicenseModifyRequest([FromBody] LicenseModifyModel model)
        {
            ...
            try
            {
                ...
                try
                {
                    var mosanafatBase = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Files", "Mosanafat");
                    ...
                    var finalFolder = Path.Combine(mosanafatBase, safeFolder);
                    ...
                    if (!string.IsNullOrWhiteSpace(tempKey) && attachInfoFromPreUpload.Count > 0)
                    {
                        ...
                        if (Directory.Exists(tempFolder))
                        {
                            ...
                            {
                                await _helperUrlApi.PostDataToApi<List<AttachVM>, ErrorMessage>(
                                    "api/Mosanafat/SaveAttachments", attachList);
                            }
                ...
                var redirect = Url.Action("LicenseModifySuccess", "Mosanafat", new { reqno = (string?)jo.SelectToken("Reqno") ?? (string?)jo.SelectToken("reqno") });
        ...
        private async Task<MoiEserviceLicensesRequestVM?> SendModifyRequestAsync(LicenseModifyModel model)
        {
            ...
            const string endpoint = "api/Mosanafat/AddModifyRequest";
            try
            {
    ...
    .ToList();
            ...
            var vm = new LicenseModifyModel
            {
                ...
                Applicant = licenseVM.Applicant , RequestFessVM = licenseVM.RequestFessVM,
        ...
        private async Task<LicencesVM?> FetchLicenseDetailsAsync(int id)
        {
            try
            {
                var url = $"api/Mosanafat/GetLicenseDetails/{id}";
        ...
        //    try
        //    {
        //        string username = _configuration["PaciData:PaciAPIUserName"];
        //        string password = _configuration["PaciData:PaciAPIPassword"];
        //        string baseUrl = _configuration["PaciData:PaciAPI"];


        //        var request = new HttpRequestMessage(HttpMethod.Post, $"{baseUrl}GetToken");
        ...
        //        var tokenResult = JsonConvert.DeserializeObject<PaciTokenResult>(content);

        //        return tokenResult?.access_token ?? "Error: token missing";
        //    }
        //    catch (Exception ex)
        //    {
        ...
        //public async Task<string> GetPaciInfoAsync(string token, string civilID)
        //{
        //    try
        //    {
        //        string apiUrl = _configuration["PaciInfoAPI"];
        //        var requestUrl = $"{apiUrl}PACI/GetPersonDetails";

        //        var requestContent = new FormUrlEncodedContent(new[]
        //        {
        //        new KeyValuePair<string, string>("civilId", civilID.Trim())
        //    });
        ...
        //        var paciJson = JsonConvert.DeserializeObject<dynamic>(responseContent);

        //        string statusCode = paciJson?.statusCode;
        ...
        //    }
        //}
        //public async Task<PaciData?> GetPaciUserInfoAsync(string token, string civilId)
        //{
        //    try
        //    {
        //        string apiUrl = _configuration["PaciData:PaciInfoAPI"];
        //        var url = $"{apiUrl}PACI/GetPersonDetails";
        ...
        //        var content = await response.Content.ReadAsStringAsync();
        //        var paciJson = JsonConvert.DeserializeObject<JObject>(content);

        //        if (paciJson == null)
        //            return null;

        //        var paciData = new PaciData
        //        {
        //            statusCode = paciJson.Value<string>("statusCode"),
        //            disclaimer = paciJson.Value<string>("disclaimer"),
        //            remainingHits = paciJson.Value<string>("remainingHits"),
        //            timeStamp = paciJson.Value<string>("timeStamp"),
        //            message = paciJson.Value<string>("message"),
        //            environment = paciJson.Value<string>("environment")
        //        };

        //        if (paciData.statusCode == "900" || paciData.statusCode == "904")
        //        {
        //            paciData.arFullName = paciJson.Value<string>("arFullName");
        //            paciData.enFullName = paciJson.Value<string>("enFullName");
        //            paciData.birthDate = paciJson.Value<string>("birthDate");
        //            paciData.email = paciJson.Value<string>("email");
        //            paciData.mobile = paciJson.Value<string>("mobile");
        //            paciData.sex = paciJson.Value<string>("sex");
        //        }
        ...
        //[HttpPost]
        //public async Task<JsonResult> GetUserDataPACI(string civilID)
        //{
        //    var token = await GetTokenAsync();

        //    if (string.IsNullOrEmpty(token))
        //    {
        //        return Json(new { success = false, message = "فشل الحصول على التوكن من PACI" });
        //    }

        //    var userData = await GetPaciUserInfoAsync(token, civilID);

        //    if (userData == null)
        //    {
        //        return Json(new { success = false, message = "فشل في جلب بيانات المستخدم" });
        //    }

        //    if (userData.statusCode == "900" || userData.statusCode == "904")
        //    {
        ...
        //            remainingHits = userData.remainingHits
        //        });
        //    }
